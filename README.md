...
├── reviews.html              # Reviews page (scoreboard + per-flavor review carousels)
├── assets/
...
H1 "DON'T TAKE OUR WORD FOR IT." → **"TONIGHT'S RATINGS" scoreboard** (giant 4.86 + blue-boxed stars + **4,000+** count + WRITE A REVIEW + VU-meter breakdown bars; powers on when scrolled into view — 4.86 counts up from 0, bars sweep up staggered, stars flicker on CRT-style, blinking `● TONIGHT'S RATINGS` OSD). **4.86 / 4,000+ are the REAL Judge.me figures** (actual ~3958, rounded even to 4,000+ — matches the homepage proof ticker).

**REORGANIZED BY FLAVOR (June 18 2026):** the old flat list + filter chips + Judge.me-widget plan are gone. The page is now:
- **Sticky flavor jump-nav** (`.rv-flavornav`, sticky under the fixed header): `▶ TUNE IN` REC-dot OSD + 6 flavor chips (`.rv-fchip`). Click → smooth-scrolls to that flavor (anchor + `scroll-margin-top`); an IntersectionObserver **scroll-spy** lights the active chip. Page-scoped `html { scroll-behavior:smooth }` (reduced-motion off). Active chip = ink fill + flavor-colored inset underline (so even yellow/green stay readable).
- **6 flavor sections** (`.rv-flavor`, CH 01–06), each a **horizontal carousel** (`.rv-car__track`, scroll-snap, hidden scrollbar; desktop arrow buttons `.rv-car__nav`, mobile native swipe) of **7 reviews + a shop-CTA end card** (`.rv-card--cta` → that flavor's PDP).
- **Flavor header (`.rv-flavor__head`):** channel chip (`CH 0X`, flavor-accent left border) + jar + flavor name on the LEFT, **SHOP ▸** on the right. The per-flavor star/count rating was **removed** (Jay, June 18).

**Reviews are HAND-CURATED REAL Judge.me reviews** (transcribed from Jay's screenshots; no live widget/API on this page — the Option-B API plan was dropped). All 6 flavors = 7 real reviews each (42 total), **verbatim** (typos, emoji, line breaks, brand names all kept). To add one: paste an `<article class="rv-card">` into the right flavor's `.rv-car__track`, CTA stays last.
- **Card (`.rv-card`):** **uniform fixed height (380px)** so the carousel never goes ragged — body clamps to 6 lines, title (`.rv-card__title`, Bebas) to 2; a steady green **`● VERIFIED`** stamp top-right (replaced the old blinking date — Jay has no exact dates); TV lower-third foot (jar + cream name + green ✓ VERIFIED BUYER · flavor) pinned bottom via `margin-top:auto`. 4-star reviews render 4 filled stars.
- **"READ MORE ▸"** (`.rv-card__more`, JS-injected only on cards whose body actually overflows the clamp) → opens the full review in a broadcast-styled **modal** (`.rv-modal`: VERIFIED REVIEW OSD, stars, full title + body, attribution; Esc/backdrop/✕ close, focus managed). Keeps the grid uniform even when expanded — the standard "equal-height card + clamp + read-more" pattern.
- **Customer-text trademark names kept VERBATIM** (Butterfinger — incl. review titles — m&ms, PB Max, Mars family, Fritos, Hostess Turtle Pies, kudos bar, etc.). **Jay's call (June 18):** the trademark-safe rule governs only N&N's OWN copy (PDP bullets etc.), NOT customer reviews. See [[feedback-nn-trademark-bullets]].
- 3 new IIFEs in `main.js` (carousel arrows + flavor scroll-spy; read-more modal), all reduced-motion-safe.
- ⚠️ **Pending:** the reviewer "ABSPALDING" (Lunch Box) was an email (`abspalding@yahoo.com`) — showing the local part only to avoid publishing a full address. *(B Diaz/Cowabunga's truncated review was completed June 18 — full verbatim text in; it's long so it clamps + uses READ MORE.)*
...
- **✅ Wired to the live bot (June 18 2026):** the modal POSTs `{clientId:"nutty-nostalgic", message, history}` to Jay's `designmynt-chatbot` Cloudflare Worker (`/chat`), renders the reply, and falls back to "text 954-275-6577" on error. The brain (system prompt) lives in the Worker; copies are in `chatbot/nn-hotline-system-prompt.md` (readable) and `chatbot/nn-hotline-worker-client.js` (the exact client config). ⚠️ The `NUTTY15` promo code still needs creating in Shopify.
...
- **Launcher yields to the PDP sticky ATC bar (June 18 2026):** on mobile product pages the sticky ADD TO CART bar (`.pdp-sticky`, fixed bottom) covered the launcher. Fix (Intercom/Drift pattern): the existing `.pdp-sticky` IntersectionObserver now also toggles `.nn-hotline--lifted`, lifting the launcher via `bottom: 88px` (not `transform` — that's reserved for hover-scale), 0.25s transition, scoped `@media max-width:767px`. Rises above the bar while it shows, drops back when it hides. (`#nn-hotline-open` is looked up *inside* the observer callback since its markup parses after `main.js`.)
- **Brain refined (June 18 2026) — `chatbot/*`:** rewrote the system prompt in **Jay's voice**, all hand-offs point to **Jay** by name (no "a human/agent"), replies cut to 1–2 sentences. Fixed bad flavor data (Cowabunga = **Hostess** Turtle Pies, vanilla-pudding **cashew** butter + green-glazed pie chips — our one cashew jar) and added real component profiles for all 6 flavors so it stops inventing. Edited both `chatbot/nn-hotline-system-prompt.md` + `chatbot/nn-hotline-worker-client.js`. ⚠️ Repo edit only — **Jay must paste the updated `nutty_nostalgic_default` block into the Cloudflare Worker to go live**, and still needs to create `NUTTY15` in Shopify.
...
- **Cart drawer (mobile):** slimmed "Complete the Set" + tightened the footer so 2–3 line items show (was ~1); `overscroll-behavior:contain` on the drawer/items/upsell-row stops scroll-chaining to the page behind. The drawer is sized off the **visible viewport** (`height: var(--vvh, 100dvh)`, same system as the hotline modal) with the **footer pinned** and the items list yielding (`min-height:0`) — so the CHECKOUT CTA is never pushed below the fold by the mobile toolbar.
...
- **Proof ticker (red marquee):** 3 phrases — "FREE SHIPPING ON ORDERS $65 OR MORE / PRE-ORDER NOW / 4,000+ 5-STAR REVIEWS" — split by a yellow ▶ (`.proof-ticker__sep`). Faster (13s loop) with a VHS tracking shudder on the bar (`ticker-tracking`) + chromatic brand-color text ghost. (Count is the real Judge.me total ~3958, rounded even to 4,000+; the reviews-page scoreboard matches.)
  - **Seamless on desktop (June 18 2026):** one copy (~1103px) was narrower than desktop viewports, so the old 2-copy `translateX(-50%)` loop showed a blank red gap before repeating. Now the track is **5 identical segments** and `proof-scroll` scrolls **`-20%` (= one segment)** at the same 13s — 4 segments always cover the screen (seamless to ~4400px), speed unchanged. Keep the segment count and the `-20%` in sync.
